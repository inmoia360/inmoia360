...
export default function BreadLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/admin/dailybread/login', {
      method: 'POST',
    ...
    setLoading(false);
    if (res.ok) {
      router.push('/delagala/dailybread/admin');
    } else {
    ...
    <main style={{ minHeight: '100vh', background: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      <div style={{ background: '#fff', borderRadius: 16, maxWidth: 360, width: '100%', padding: '2.5rem 2rem', boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 48, height: 48, background: '#E0A52C', borderRadius: 10, fontSize: '1.6rem', fontWeight: 900, color: '#fff', marginBottom: '.5rem' }}>D</div>
          <div style={{ fontSize: '1.1rem', letterSpacing: '4px', fontWeight: 800 }}>DELAGALA</div>
          <div style={{ fontSize: '.65rem', color: '#999', letterSpacing: '2px' }}>PAN · ADMIN</div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width: '100%', padding: '.85rem', background: loading ? '#ccc' : '#E0A52C', color: '#fff', border: 'none', borderRadius: 8, fontSize: '1rem', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', letterSpacing: '1px' }}
          >
